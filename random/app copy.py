...
from flask import Flask, request, redirect, session, send_from_directory, url_for, jsonify
...
# For demo purposes only; in production load from environment
...
def signup():
	username = (request.form.get("username") or "").strip()
	password = (request.form.get("password") or "").strip()
	# Validate inputs and report specific errors
	if not username:
